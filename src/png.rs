...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let chunk_type_from_str = ChunkType::from_str(chunk_type)?;

        for i in 0..self.chunks.len() {
            if *self.chunks[i].chunk_type() == chunk_type_from_str {
                let rc = self.chunks.remove(i);
                return Ok(rc);
            }
        }

        Err("No chunk found!".into())
    }
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        let chunk_type_from_str = ChunkType::from_str(chunk_type).unwrap();
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        for chunk in &self.chunks {
            all_chunks.extend(chunk.as_bytes());
        }

        self.header
            .iter()
            .chain(all_chunks.iter())
            .copied()
            .collect()
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        while pointer < value.len() {
            ...
            if crc_byte != crc_val {
                return Err("Invalid CRC32 IEEE/ISO for chunk!");
            }

            let chunk = Chunk::try_from(&value[pointer..=(8 + pointer + length as usize + 3)])?;
            chunks.push(chunk);

            pointer += 12 + length as usize;
        }
